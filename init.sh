...
#nginx mysql-server php-fpm php-init php-mysql

echo "Installing guest additions. Media must be mounted!"
sudo sh /media/ubuntu/VBox_GAs_6.1.32/autorun.sh

echo "Installing web stack"
sudo apt install nginx mysql-server php-fpm php-init php-mysql
...
#ПЕРЕНОСИМ КОНФИГ В ПАПКУ nginx, т.е. создаем ссылку
sudo ln -s /www/phpdev-01/conf/pma.conf /etc/nginx/sites-enabled/pma

#добавляем хост в файл /etc/hosts
#добавляем 2 пустые строки и 3 коммента, чтобы отделить от других строк
echo "\n\n###" | sudo tee -a /etc/hosts
echo "127.0.0.1\tpma.my" | sudo tee -a /etc/hosts

# Git config
echo "Git config"
git config --global user.email "aykyshar@gmail.com"
git config --global user.name "ai-kys"
